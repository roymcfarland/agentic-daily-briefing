...
describe("smoke: morning-brief cron route preview", () => {
  beforeAll(() => {
    ...
    // need real Taskflow / Resend / Redis credentials because preview mode
    // bypasses the actual send and idempotency lock paths.
    process.env.CRON_SECRET = TEST_CRON_SECRET;
    process.env.TASKFLOW_API_BASE_URL = process.env.TASKFLOW_API_BASE_URL ?? "https://example.invalid";
    process.env.READ_ONLY_API_KEY = process.env.READ_ONLY_API_KEY ?? "tf_smoke_test";
