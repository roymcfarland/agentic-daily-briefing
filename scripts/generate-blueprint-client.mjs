...
const categoryIsEnum = Array.isArray(taskCategoryEnum) && taskCategoryEnum.length > 0;

function kebabToCamel(value) {
  return value.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
}

const taskCategoryValues = categoryIsEnum
  ? taskCategoryEnum.map((value) => `"${value}"`).join(" | ")
  : "string";

const byCategoryType = categoryIsEnum
  ? `{\n${taskCategoryEnum
      .map((value) => `      ${kebabToCamel(value)}?: number;`)
      .join("\n")}\n    }`
  : "Record<string, number>";

const isTaskCategoryBody = categoryIsEnum
  ? taskCategoryEnum
      .map((value, index) => {
